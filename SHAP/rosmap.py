...
config = {
    "result_dir": "/path/to/results/svm_regression_grid_search_cv_rosmap_all/",
    "model_path": "/path/to/models/svm_regression_grid_search_cv_rosmap_all/4000_fold_5.joblib",
    "data_file": "/path/to/exp_rosmap_with.resilience_cor_seqbatch_4000.csv",
...
X = df.iloc[:, :4000].values  # Use the first 4000 features
...
feature_names = df.columns[:4000]

# Load gene mapping data
gene_mapping = pd.read_csv(config["gene_mapping_file"])
# Create a dictionary for Ensembl ID to gene symbol mapping, excluding missing values
...
# Extract test indices for the selected fold (fold=5 in this case)
for fold, (train_index, test_index) in enumerate(kf.split(X_scaled)):
    if fold == 4:  # Corresponds to the 5th fold (index 4)
...
def plot_shap_summary(model, X, feature_names, result_dir):
    ...
    plt.savefig(os.path.join(result_dir, 'shap_summary_4000_fold_5.png'))
    plt.close()
    
    # Save SHAP values to CSV
    shap_values_df = pd.DataFrame(shap_values, columns=feature_names)
    shap_values_df.to_csv(os.path.join(result_dir, 'shap_values_4000_fold_5.csv'), index=False)
